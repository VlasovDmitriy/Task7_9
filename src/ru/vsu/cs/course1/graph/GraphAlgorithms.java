...
public class GraphAlgorithms {
    ...
    public static void solution(Graph graph, int[] vertices, int from, int to) {
        for(Integer i: bfs(graph, 0)) {
            for(int k: vertices) {
                if(i == k) {
                    for(Integer x: graph.adjacencies(k)) {
                        graph.removeAdge(x, k);
                    }
                }
            }
        }
        System.out.println("Путь от 0 до 4, избегая 7, 6, 3: ");
        ArrayList<Integer> path = new ArrayList<>();
        for (Integer v : GraphAlgorithms.dfs(graph, to)) {
            if(v == from) {
                path.add(v);
                break;
            }
